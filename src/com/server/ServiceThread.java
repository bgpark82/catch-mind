...
public class ServiceThread extends Thread {
	...
	public int Auth, min, sec, mili, userScore, score;

	private StringTokenizer str, st;
	public String userName, message, protocol, msg, tempans;
	...
	switchRandomAuth switchRandomAuth;

	public ServiceThread(ChatServer server, Socket socket, int Auth) {
		this.server = server;
		this.socket = socket;
		this.Auth = Auth;
	}
...
//			sendMessage("DragMouse/");
			...
			case "Timer":
				...
				switchRandomAuth = new switchRandomAuth();
				switchRandomAuth.start();
				...
				if (outName.indexOf(msg) == -1 && !msg.equals(userName)) {
					for (int i = 0; i < server.Clients.size(); i++) {
						if (msg.equals(server.Clients.elementAt(i).userName)) {
							...
							sendMessage("Out/당신은" + msg + "님에게 강퇴표를 날렸습니다 ㅋ");
							server.userOut(i);
						}
					}
				} else if (msg.equals(userName)) {
					sendMessage("Out/* 자신을 강퇴 할 수 없습니다. ");
				} else if (outName.indexOf(msg) != -1)
					sendMessage("Out/" + msg + "님에 대한 강퇴표를 이미 사용했습니다. ");
				break;
	...
	public void cochau() {
		for (int i = 0; i < server.Clients.size(); i++) {
			if (server.Clients.get(i).Auth == 1) {
				server.Clients.get(i).switchRandomAuth.killa();
			}
		}
	}
