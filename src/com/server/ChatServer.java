...
public class ChatServer {
	...
	public void firstAuthUser() throws IOException {
		if ((Clients.get(0).Auth == 1)) {
			for (int i = 1; i < Clients.size(); i++) {
				Clients.get(i).Auth = 0;
				ServiceThread st = ((ServiceThread) Clients.elementAt(i));
	...
	public void userOut(int i) {
		...
		try {
			...
			sendMessageAll("Chatting/* 주의  * " + st.getUserName() + "님 강퇴표를 받았습니다 (현재 " + st.getOutNum() + "표     "
					+ (Clients.size() - 1) + "표 되면 강퇴당합니다)");

			if (st.getOutNum() == (Clients.size() - 1)) {
...
//	public void correctAuthUser(String username) throws IOException {
//
//		for(int i = 0; i < Clients.size(); i++) {
//		System.out.println("client : " + Clients);	
//		Clients.get(i).Auth = 0;
//		System.out.println("Clients username : " + Clients.get(i).userName);
//		System.out.println("Clients index : " + Clients.indexOf(Clients.get(i).userName));
//		System.out.println("Clients size : " + Clients.size());
//		}
//		Clients.get(Clients.indexOf(username)).Auth = 1;
//		sendMessageAll("unAuth/권한을 바꾼 다음 버튼을 disable해준다.");
//		Clients.get(Clients.indexOf(username)).sendMessage("Auth/권한을 부여해준다.");	
//		System.out.println("new authority number : " + Clients.indexOf(username) + "\n new authrized thread : " + Clients.get(Clients.indexOf(username)));
//}
	...
	public void testRank() {
		for (ServiceThread userList : Rank) { // 각 유저당 한번만 실행되면 됨.
			userList.setScore((int) (Math.random() * 100));
		}
	}
	...
	public static void main(String[] args) {
		...
		try {
			while (true) {
				...
				ServiceThread thread = new ServiceThread(server, serviceSocket, Auth);
				thread.start();
